...
# Contrib   : Jan Jungmann, Raphaël Bordet
# TODO :
#################################################################
#
...
my $aix_swap_total      = "1.3.6.1.4.1.2.6.191.2.4.2.1.4";  # aixPageSize
...
my $aix_swap_index      = "1.3.6.1.4.1.2.6.191.2.4.2.1.8";  # aixPageIndex
...
my $o_community = undef;    # SNMPv2 community
my $o_port      = 161;      # SNMP port
my $o_help      = undef;    # wan't some help ?
my $o_verb      = undef;    # verbose mode
my $o_version   = undef;    # print version
my $o_type      = "netsnmp";# Remote SNMP agent type (default: Net-SNMP)
...
sub p_version {
    print "check_snmp_mem version : $Version\n";
}
...
sub help {
    ...
    print "(c)2004-2013 - Authors: Patrick Proy, Raphaël Bordet\n\n";
    print_usage();
    print <<EOT;
-v, --verbose
...
-T, --type
   SNMP Agent type (netsnmp, cisco, hp or aix. Default: netsnmp)
-m, --memcache
...
sub check_options {
    ...
    if ( $o_type !~ m/(netsnmp|cisco|hp|aix)/ ) {
        print_usage();
        exit $ERRORS{"UNKNOWN"};
    }
    verb("Type = $o_type");
...
if ( $o_type eq "hp" ) {
    ...
    foreach (@index) {
        ...
        $c_output .= "Slot " . $$resultat{$hp_mem_index . "." . $_} . ":"
                    .round( 100 - ($$resultat{$hp_mem_free . "."
                    . $_} *100 / $$resultat{$hp_mem_total . "." . $_}) ,0) . "%";
    }
    my $used = $total - $free;
    $prct_free=round($used*100/($total),0);
...
if ( $o_type eq "aix" ) {
    ...
    foreach (@index) {
        ...
        if ( $$resultat{$aix_swap_status . "." .$_} == 2) {
            $nactive++; 
            $c_output .= ", " if defined ($c_output);
            $prct_free = 100-$$resultat{$aix_swap_usage . "." .$_};
            $used = $$resultat{$aix_swap_usage . "." .$_};
            $c_output .= $$resultat{$aix_swap_name . "." .$_} .": ". $used ."%";
            $free = ($prct_free*$$resultat{$aix_swap_total . "." .$_})/100;
            $total = $free+$used;
            verb("Used : $used, Free: $free");
            ...
            if (defined ($o_perf)) {
                ...
                $c_foutput .= "page_space_"
                            . $$resultat{$aix_swap_name . "." .$_}
                            ."_used=" . $used.";";
